...
function getCursorCss(
    ...
    return (
        `url("data:image/svg+xml,<svg height='32' width='32' viewBox='0 0 32 32' xmlns='http://www.w3.org/2000/svg' style='color: ${color};'><defs><filter id='shadow' y='-40%' x='-40%' width='180px' height='180%' color-interpolation-filters='sRGB'><feDropShadow dx='${dx}' dy='${dy}' stdDeviation='1.2' flood-opacity='.5'/></filter></defs><g fill='none' transform='rotate(${r + tr
...
type CursorKey =
    ...
    | "senw-rotate"
    | "swne-rotate";
const CURSORS: Record<CursorKey, CursorFunction> = {
    none: () => 'none',
...
export function getCursor(cursor: CursorKey, rotation = 0, color = 'black') {
    return CURSORS[cursor](rotation, false, color)
}